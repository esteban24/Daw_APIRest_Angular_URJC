...
function FootballManager($resource, $timeout, $http) {
	...
	vm.modifyingtournament = function(tournament) {
		$id = tournament.id;
		TournamentModifyResource.update({
			id : $id
		}, tournament);
	}
	...
	vm.deleteTournament = function(tournament) {
		$id = tournament.id;
		TournamentResource.deleteTournament({
			id : $id
		});
	}
	...
	vm.deleteTeam = function(team) {
		$id = team.id;
		TeamResource.deleteTeam({
			id : $id
		});
	}

	vm.login = function($mail, $pass, callback) {
		$http.post("/adminLogin/seguridad", {
			mail : $mail,
			pass : $pass
		}).success(callback);
	}
	...
	vm.loginuser = function($mailuser, $passuser, callback) {
		$http.post("/adminLogin/seguridadUsuario", {
			mail : $mailuser,
			pass : $passuser
		}).success(callback);
	}
